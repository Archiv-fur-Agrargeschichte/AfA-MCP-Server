const I18N = {
  de: {
    ...
    templates_h: 'Fertige Vorlagen im Assistenten',
    templates: '<p>Neben den Werkzeugen liefert der Server sechs <strong>Vorlagen</strong> mit. Eine Vorlage ist ein vorformulierter Auftrag: Sie wählen sie aus, geben eine Frage oder einen Namen dazu, und der Assistent erhält damit nicht nur Ihr Anliegen, sondern auch die Arbeitsanweisung, wie er suchen und ausgeben soll. Das ist der Unterschied zur freien Frage, bei der er Werkzeug, Suchbegriffe, Reihenfolge und Umfang selbst wählt.</p><p>Die Vorlagen kommen vom Server, nicht aus einer Datei. Sie müssen nichts kopieren, und wenn wir eine Vorlage verbessern, arbeiten Sie beim nächsten Start mit der neuen Fassung.</p><table><thead><tr><th>Vorlage</th><th>Wofür</th><th>Angaben</th></tr></thead><tbody><tr><td class="tool">recherche_belegt</td><td>Frage beantworten, mit Dokument-ID, Link und Protokoll der Suchaufrufe</td><td class="tool">frage</td></tr><tr><td class="tool">trefferliste_vollstaendig</td><td>Alle Treffer einer Anfrage aufzählen, ohne Auswahl</td><td class="tool">query, hierarchy</td></tr><tr><td class="tool">entity_dossier</td><td>Person, Institution oder Betrieb nach festem Formular</td><td class="tool">name, typ</td></tr><tr><td class="tool">av_quellenliste</td><td>Fotos und Filme mit Jahr, Dauer, Auftraggeber, Dossiernummer</td><td class="tool">thema, zeitraum</td></tr><tr><td class="tool">widerspruchspruefung</td><td>Angaben mehrerer Einträge gegenüberstellen, ohne sie aufzulösen</td><td class="tool">sachverhalt, ids</td></tr><tr><td class="tool">fremdpruefung</td><td>Ein fremdes Ergebnis blind nachrechnen und Abweichungen benennen</td><td class="tool">rezept</td></tr></tbody></table><p>Alle Vorlagen kennen zusätzlich die Angabe <code>language</code> mit <code>de</code> oder <code>en</code>. Ohne Angabe antworten sie auf Deutsch.</p><h3>So rufen Sie eine Vorlage auf</h3><ul><li><strong>Claude Desktop und claude.ai:</strong> im Eingabefeld auf das Pluszeichen, dann den Server «afa-recherche» wählen. Die sechs Vorlagen erscheinen als Liste; nach der Auswahl fragt Claude die Angaben ab.</li><li><strong>Claude Code:</strong> als Schrägstrich-Befehl, etwa <code>/mcp__afa-recherche__entity_dossier</code>.</li><li><strong>VS Code und andere Entwicklungsumgebungen:</strong> im Chat über die Vorlagenliste des Servers, je nach Version als <code>/mcp.afa-recherche.recherche_belegt</code>.</li><li><strong>Clients ohne Vorlagen-Unterstützung</strong> (derzeit etwa ChatGPT und Perplexity): Die Werkzeuge funktionieren dort normal. Den Text einer Vorlage können Sie aus <a href="https://github.com/Archiv-fur-Agrargeschichte/AfA-MCP-Server/tree/main/docs/prompts">docs/prompts</a> in den Chat kopieren.</li></ul><p>Zwei Hinweise zur Erwartung. Erstens sind die Vorlagen streng und deshalb unbequem: Die vollständige Trefferliste zählt auch offensichtlich unpassende Treffer auf, das Dossier gibt leere Felder aus, und die Widerspruchsprüfung entscheidet nichts. Das ist beabsichtigt, denn genau dort entstehen sonst stille Auslassungen. Zweitens verringern sie die Streuung zwischen zwei Läufen, sie beseitigen sie nicht: Auswahl und Formulierung des Modells bleiben variabel.</p>',
    sysprompt_h: 'Systemprompt des Servers',
    sysprompt: '<p>Ein MCP-Server liefert dem Client bei jeder Verbindung nicht nur Werkzeuge, sondern auch einen Text mit Anweisungen (Feld <code>instructions</code>). Der Client stellt ihn dem Sprachmodell voran. Er entscheidet damit mit, wie sorgfältig ein Assistent arbeitet, ohne dass Nutzende etwas einrichten müssen.</p><p>Der AfA-Server beschreibt darin die Bestände und die Werkzeuge und liefert die Regeln auf Deutsch und Englisch mit, je zehn, die die Streuung der Antworten verringern: jede Aussage mit Dokument-ID und Link belegen, nicht aus dem gekürzten Trefferausschnitt antworten sondern den Eintrag abrufen, für wiederholbare Ergebnisse nach ID sortieren statt nach Relevanz, vor der Aussage «alle Treffer» bis zum Ende der Trefferliste blättern, alle im Eintrag genannten Ämter aufführen statt der wichtigsten, und Widersprüche zwischen Einträgen mit beiden Kennungen nennen statt sie aufzulösen. Auch damit bleiben Auswahl und Formulierung des Assistenten variabel: die Regeln verringern Auslassungen und Zufall, sie beseitigen sie nicht.</p><p>Neben dem Systemprompt liefert der Server sechs Vorlagen über das Prompts-Primitive von MCP aus (<code>prompts/list</code> und <code>prompts/get</code>), siehe <a href="#vorlagen">Fertige Vorlagen im Assistenten</a>. Der Unterschied: Der Systemprompt gilt immer und ungefragt, eine Vorlage wählt eine Person bewusst aus. Wortlaut der Regeln in <code>SERVER_INSTRUCTIONS</code> (<code>src/afa_mcp/server.py</code>), Vorlagen in <code>src/afa_mcp/prompts.py</code>, Herleitung in <a href="https://github.com/Archiv-fur-Agrargeschichte/AfA-MCP-Server/tree/main/docs/prompts">docs/prompts</a>.</p>',
    ...
    reliability: '<p>KI-Assistenten sind ein Recherchewerkzeug, keine Quelle. Zwei Eigenschaften sollten Sie als Historikerin oder Historiker im Blick behalten:</p><p><strong>Reproduzierbarkeit:</strong> Stellen Sie dieselbe Frage zweimal, kann der Assistent unterschiedliche Suchanfragen formulieren, andere Treffer auswählen und zu anderen Schwerpunkten in der Zusammenfassung kommen. Was Sie heute finden, findet eine Kommilitonin morgen mit derselben Frage vielleicht nicht. Halten Sie deshalb fest, was zählt: die Dokument-IDs und die Links zu den Quellen-Einträgen. Diese sind stabil und zitierfähig, der Chatverlauf ist es nicht.</p><p><strong>Was hilft.</strong> Je genauer die Eingabe, desto wiederholbarer das Ergebnis. Eine Frage in natürlicher Sprache lässt dem Assistenten viele Entscheidungen offen (Werkzeug, Suchbegriffe, Filter). Ein explizit notierter Suchaufruf wie <code>search_entities({"entity_type": "person", "query": "Ineichen"})</code> legt diese Entscheidungen fest und liefert bei unverändertem Bestand dieselben Treffer. Empfehlung: beim Herumstöbern frei fragen, für zentrale Befunde aber den tatsächlich ausgeführten Suchaufruf aus dem Chatverlauf kopieren und dokumentieren.</p><p>Wer volle Reproduzierbarkeit braucht, ruft die Schnittstelle direkt per Skript auf (siehe technische Referenz).</p><p>Kurz: Der Assistent ersetzt die Suchmaske, nicht die Quellenkritik.</p>',
    ...
    tool_fetch: 'Metadaten eines einzelnen Dokuments anhand seiner ID.',
  ...
  fr: {
    ...
    templates_h: 'Modèles prêts à l’emploi dans l’assistant',
    templates: '<p>Outre les outils, le serveur fournit six <strong>modèles</strong>. Un modèle est une consigne pré-rédigée : vous le choisissez, vous ajoutez une question ou un nom, et l’assistant reçoit non seulement votre demande mais aussi la manière de chercher et de restituer. C’est la différence avec une question libre, où il choisit lui-même l’outil, les termes, l’ordre et l’étendue.</p><p>Les modèles viennent du serveur, pas d’un fichier. Vous n’avez rien à copier, et si nous améliorons un modèle, vous travaillez avec la nouvelle version au prochain démarrage.</p><table><thead><tr><th>Modèle</th><th>Pour quoi</th><th>Paramètres</th></tr></thead><tbody><tr><td class="tool">recherche_belegt</td><td>Répondre à une question, avec identifiant, lien et journal des appels</td><td class="tool">frage</td></tr><tr><td class="tool">trefferliste_vollstaendig</td><td>Énumérer tous les résultats d’une requête, sans sélection</td><td class="tool">query, hierarchy</td></tr><tr><td class="tool">entity_dossier</td><td>Personne, institution ou exploitation selon un formulaire fixe</td><td class="tool">name, typ</td></tr><tr><td class="tool">av_quellenliste</td><td>Photos et films avec année, durée, commanditaire, numéro de dossier</td><td class="tool">thema, zeitraum</td></tr><tr><td class="tool">widerspruchspruefung</td><td>Comparer les indications de plusieurs notices sans les trancher</td><td class="tool">sachverhalt, ids</td></tr><tr><td class="tool">fremdpruefung</td><td>Refaire à l’aveugle un résultat étranger et nommer les écarts</td><td class="tool">rezept</td></tr></tbody></table><p>Tous les modèles acceptent en plus le paramètre <code>language</code> avec <code>de</code> ou <code>en</code>. Sans indication, ils répondent en allemand.</p><h3>Comment appeler un modèle</h3><ul><li><strong>Claude Desktop et claude.ai :</strong> dans le champ de saisie, cliquez sur le signe plus, puis choisissez le serveur « afa-recherche ». Les six modèles apparaissent en liste ; après la sélection, Claude demande les paramètres.</li><li><strong>Claude Code :</strong> comme commande slash, par exemple <code>/mcp__afa-recherche__entity_dossier</code>.</li><li><strong>VS Code et autres environnements de développement :</strong> dans le chat via la liste des modèles du serveur, selon la version sous la forme <code>/mcp.afa-recherche.recherche_belegt</code>.</li><li><strong>Clients sans prise en charge des modèles</strong> (actuellement ChatGPT et Perplexity par exemple) : les outils y fonctionnent normalement. Vous pouvez copier le texte d’un modèle depuis <a href="https://github.com/Archiv-fur-Agrargeschichte/AfA-MCP-Server/tree/main/docs/prompts">docs/prompts</a> dans le chat.</li></ul><p>Deux remarques sur ce qu’il faut attendre. D’abord, les modèles sont stricts et donc inconfortables : la liste complète énumère aussi des résultats manifestement hors sujet, le dossier affiche des champs vides et l’examen des contradictions ne tranche rien. C’est voulu, car c’est précisément là que naissent les omissions silencieuses. Ensuite, ils réduisent la dispersion entre deux exécutions, ils ne la suppriment pas : la sélection et la formulation du modèle restent variables.</p>',
    sysprompt_h: 'Prompt système du serveur',
    sysprompt: '<p>Un serveur MCP ne fournit pas seulement des outils au client, mais aussi un texte d\'instructions (champ <code>instructions</code>) transmis à chaque connexion. Le client le place devant le modèle de langue. Il influence donc le soin avec lequel un assistant travaille, sans que l\'utilisateur ait à configurer quoi que ce soit.</p><p>Le serveur AfA y décrit les fonds et les outils et transmet les règles en allemand et en anglais, dix de chaque, visant à réduire la dispersion des réponses : étayer chaque affirmation par un identifiant de document et un lien, ne pas répondre à partir de l\'extrait abrégé mais consulter la notice, trier par identifiant plutôt que par pertinence pour des résultats répétables, parcourir la liste jusqu\'au bout avant de parler de « tous les résultats », citer toutes les fonctions figurant dans la notice et non les principales, et signaler les contradictions entre notices avec les deux identifiants au lieu de les trancher. Même ainsi, la sélection et la formulation de l\'assistant restent variables : les règles réduisent les omissions et le hasard, elles ne les suppriment pas.</p><p>Outre le prompt système, le serveur fournit six modèles via le primitive Prompts de MCP (<code>prompts/list</code> et <code>prompts/get</code>), voir <a href="#vorlagen">Modèles prêts à l’emploi</a>. La différence : le prompt système s’applique toujours et sans être demandé, un modèle est choisi délibérément. Texte des règles dans <code>SERVER_INSTRUCTIONS</code> (<code>src/afa_mcp/server.py</code>), modèles dans <code>src/afa_mcp/prompts.py</code>, justification dans <a href="https://github.com/Archiv-fur-Agrargeschichte/AfA-MCP-Server/tree/main/docs/prompts">docs/prompts</a>.</p>',
    ...
    reliability: '<p>Les assistants IA sont un outil de recherche, pas une source. Deux caractéristiques méritent l’attention de l’historien ou de l’historienne :</p><p><strong>Reproductibilité :</strong> posez deux fois la même question et l’assistant peut formuler d’autres requêtes, sélectionner d’autres résultats et mettre d’autres accents dans son résumé. Ce que vous trouvez aujourd’hui, une collègue ne le trouvera peut-être pas demain avec la même question. Notez donc ce qui compte : les identifiants de documents et les liens vers les notices sources. Ceux-ci sont stables et citables, la conversation ne l’est pas.</p><p><strong>Ce qui aide.</strong> Plus la saisie est précise, plus le résultat est répétable. Une question en langage courant laisse à l’assistant de nombreuses décisions (outil, termes de recherche, filtres). Une requête notée explicitement comme <code>search_entities({"entity_type": "person", "query": "Ineichen"})</code> fixe ces décisions et livre les mêmes résultats tant que les fonds n’ont pas changé. Recommandation : questionner librement pour fureter, mais copier et documenter la requête réellement exécutée depuis la conversation pour les résultats centraux.</p><p>Pour une reproductibilité totale, appelez l’interface directement par script (voir la référence technique).</p><p>En bref : l’assistant remplace le masque de recherche, pas la critique des sources.</p>',
    ...
    integration_p: 'Cliquez ci-dessous sur le client concerné pour les instructions détaillées.',
    ...
    tool_fetch: 'Métadonnées d\'un document individuel à partir de son identifiant.',
  ...
  it: {
    ...
    templates_h: 'Modelli pronti nell’assistente',
    templates: '<p>Oltre agli strumenti, il server fornisce sei <strong>modelli</strong>. Un modello è un incarico già formulato: lo selezionate, aggiungete una domanda o un nome e l’assistente riceve non solo la vostra richiesta ma anche l’istruzione su come cercare e restituire. È questa la differenza rispetto a una domanda libera, dove sceglie da sé strumento, termini, ordine ed estensione.</p><p>I modelli arrivano dal server, non da un file. Non dovete copiare nulla e, se miglioriamo un modello, al prossimo avvio lavorate con la nuova versione.</p><table><thead><tr><th>Modello</th><th>Per cosa</th><th>Parametri</th></tr></thead><tbody><tr><td class="tool">recherche_belegt</td><td>Rispondere a una domanda, con identificatore, link e registro delle chiamate</td><td class="tool">frage</td></tr><tr><td class="tool">trefferliste_vollstaendig</td><td>Elencare tutti i risultati di una query, senza selezione</td><td class="tool">query, hierarchy</td></tr><tr><td class="tool">entity_dossier</td><td>Persona, istituzione o azienda secondo un formulario fisso</td><td class="tool">name, typ</td></tr><tr><td class="tool">av_quellenliste</td><td>Foto e film con anno, durata, committente, numero di dossier</td><td class="tool">thema, zeitraum</td></tr><tr><td class="tool">widerspruchspruefung</td><td>Confrontare le indicazioni di più voci senza risolverle</td><td class="tool">sachverhalt, ids</td></tr><tr><td class="tool">fremdpruefung</td><td>Ricalcolare alla cieca un risultato altrui e nominare gli scostamenti</td><td class="tool">rezept</td></tr></tbody></table><p>Tutti i modelli accettano inoltre il parametro <code>language</code> con <code>de</code> o <code>en</code>. Senza indicazione rispondono in tedesco.</p><h3>Come richiamare un modello</h3><ul><li><strong>Claude Desktop e claude.ai:</strong> nel campo di immissione premete il segno più e scegliete il server «afa-recherche». I sei modelli compaiono come elenco; dopo la scelta Claude chiede i parametri.</li><li><strong>Claude Code:</strong> come comando slash, ad esempio <code>/mcp__afa-recherche__entity_dossier</code>.</li><li><strong>VS Code e altri ambienti di sviluppo:</strong> nella chat tramite l’elenco dei modelli del server, a seconda della versione come <code>/mcp.afa-recherche.recherche_belegt</code>.</li><li><strong>Client senza supporto per i modelli</strong> (attualmente per esempio ChatGPT e Perplexity): gli strumenti funzionano normalmente. Il testo di un modello si può copiare nella chat da <a href="https://github.com/Archiv-fur-Agrargeschichte/AfA-MCP-Server/tree/main/docs/prompts">docs/prompts</a>.</li></ul><p>Due avvertenze sulle aspettative. Primo, i modelli sono severi e quindi scomodi: l’elenco completo riporta anche risultati palesemente inadatti, il dossier stampa campi vuoti e la verifica delle contraddizioni non decide nulla. È voluto, perché è lì che nascono le omissioni silenziose. Secondo, riducono la dispersione fra due esecuzioni, non la eliminano: selezione e formulazione del modello restano variabili.</p>',
    sysprompt_h: 'Prompt di sistema del server',
    sysprompt: '<p>Un server MCP non fornisce al client soltanto strumenti, ma anche un testo di istruzioni (campo <code>instructions</code>) consegnato a ogni connessione. Il client lo anteponde al modello linguistico e influisce quindi sulla cura con cui un assistente lavora, senza che l\'utente debba configurare nulla.</p><p>Il server AfA vi descrive i fondi e gli strumenti e trasmette le regole in tedesco e in inglese, dieci per lingua, volte a ridurre la dispersione delle risposte: documentare ogni affermazione con identificatore e link, non rispondere partendo dall\'estratto abbreviato ma consultare la voce, ordinare per identificatore invece che per pertinenza per risultati ripetibili, scorrere l\'elenco fino in fondo prima di parlare di «tutti i risultati», elencare tutte le cariche riportate nella voce e non le principali, e indicare le contraddizioni fra voci con entrambi gli identificatori invece di risolverle. Anche così selezione e formulazione dell\'assistente restano variabili: le regole riducono omissioni e casualità, non le eliminano.</p><p>Oltre al prompt di sistema, il server fornisce sei modelli tramite il primitivo Prompts di MCP (<code>prompts/list</code> e <code>prompts/get</code>), vedi <a href="#vorlagen">Modelli pronti</a>. La differenza: il prompt di sistema vale sempre e senza richiesta, un modello viene scelto deliberatamente. Testo delle regole in <code>SERVER_INSTRUCTIONS</code> (<code>src/afa_mcp/server.py</code>), modelli in <code>src/afa_mcp/prompts.py</code>, motivazione in <a href="https://github.com/Archiv-fur-Agrargeschichte/AfA-MCP-Server/tree/main/docs/prompts">docs/prompts</a>.</p>',
    ...
    reliability: '<p>Gli assistenti IA sono uno strumento di ricerca, non una fonte. Due caratteristiche meritano l’attenzione dello storico o della storica:</p><p><strong>Riproducibilità:</strong> ponete due volte la stessa domanda e l’assistente può formulare interrogazioni diverse, selezionare altri risultati e porre accenti diversi nel riassunto. Ciò che trovate oggi, una collega potrebbe non trovarlo domani con la stessa domanda. Annotate quindi ciò che conta: gli identificativi dei documenti e i link alle voci di origine. Questi sono stabili e citabili, la conversazione no.</p><p><strong>Che cosa aiuta.</strong> Più precisa è l’immissione, più ripetibile è il risultato. Una domanda in linguaggio corrente lascia all’assistente molte decisioni (strumento, termini di ricerca, filtri). Un’interrogazione annotata esplicitamente come <code>search_entities({"entity_type": "person", "query": "Ineichen"})</code> fissa queste decisioni e restituisce gli stessi risultati finché i fondi non cambiano. Raccomandazione: chiedere liberamente mentre si curiosa, ma per i risultati centrali copiare e documentare l’interrogazione realmente eseguita dalla conversazione.</p><p>Chi ha bisogno di piena riproducibilità chiama l’interfaccia direttamente via script (vedi riferimento tecnico).</p><p>In breve: l’assistente sostituisce la maschera di ricerca, non la critica delle fonti.</p>',
    ...
    integration_p: 'Cliccate qui sotto sul rispettivo client per le istruzioni dettagliate.',
    ...
    tool_fetch: 'Metadati di un singolo documento tramite il suo ID.',
  ...
  en: {
    ...
    templates_h: 'Ready-made templates in the assistant',
    templates: '<p>Besides the tools, the server ships six <strong>templates</strong>. A template is a pre-formulated assignment: you pick it, add a question or a name, and the assistant receives not only your request but also the working instructions for how to search and how to present the result. That is the difference from a free-form question, where it picks tool, terms, ordering and scope on its own.</p><p>The templates come from the server, not from a file. There is nothing to copy, and when we improve a template you work with the new version the next time you start.</p><table><thead><tr><th>Template</th><th>What for</th><th>Arguments</th></tr></thead><tbody><tr><td class="tool">recherche_belegt</td><td>Answer a question, with document ID, link and a log of the search calls</td><td class="tool">frage</td></tr><tr><td class="tool">trefferliste_vollstaendig</td><td>Enumerate all hits of a query, with no selection</td><td class="tool">query, hierarchy</td></tr><tr><td class="tool">entity_dossier</td><td>Person, institution or farm following a fixed form</td><td class="tool">name, typ</td></tr><tr><td class="tool">av_quellenliste</td><td>Photos and films with year, duration, commissioning body, dossier number</td><td class="tool">thema, zeitraum</td></tr><tr><td class="tool">widerspruchspruefung</td><td>Set statements from several records side by side without resolving them</td><td class="tool">sachverhalt, ids</td></tr><tr><td class="tool">fremdpruefung</td><td>Redo somebody else’s result blind and name the deviations</td><td class="tool">rezept</td></tr></tbody></table><p>Every template also takes the argument <code>language</code> with <code>de</code> or <code>en</code>. Without it they answer in German.</p><h3>How to call a template</h3><ul><li><strong>Claude Desktop and claude.ai:</strong> click the plus sign in the input field, then pick the «afa-recherche» server. The six templates appear as a list; once selected, Claude asks for the arguments.</li><li><strong>Claude Code:</strong> as a slash command, for instance <code>/mcp__afa-recherche__entity_dossier</code>.</li><li><strong>VS Code and other development environments:</strong> in the chat through the server’s template list, depending on the version as <code>/mcp.afa-recherche.recherche_belegt</code>.</li><li><strong>Clients without template support</strong> (currently ChatGPT and Perplexity, for example): the tools work there as usual. You can copy the text of a template from <a href="https://github.com/Archiv-fur-Agrargeschichte/AfA-MCP-Server/tree/main/docs/prompts">docs/prompts</a> into the chat.</li></ul><p>Two notes on what to expect. First, the templates are strict and therefore inconvenient: the complete result list also enumerates obviously irrelevant hits, the dossier prints empty fields, and the contradiction check decides nothing. That is deliberate, because those are exactly the places where silent omissions arise. Second, they narrow the spread between two runs, they do not remove it: the model’s selection and phrasing stay variable.</p>',
    sysprompt_h: 'System prompt of the server',
    sysprompt: '<p>An MCP server hands the client not only tools but also a text of instructions (field <code>instructions</code>) on every connection. The client places it in front of the language model, so it shapes how carefully an assistant works without anyone having to configure a thing.</p><p>The AfA server uses it to describe the holdings and the tools and ships the rules in German and English, ten each, that narrow the spread of answers: back every statement with a document ID and link, do not answer from the shortened result snippet but fetch the record, sort by ID rather than relevance when results must be repeatable, page to the end of the result list before saying "all hits", list every office named in the record rather than the main ones, and report contradictions between records with both identifiers instead of resolving them. Even then the assistant\'s selection and phrasing stay variable: the rules reduce omissions and chance, they do not remove them.</p><p>Besides the system prompt the server ships six templates through MCP\'s prompts primitive (<code>prompts/list</code> and <code>prompts/get</code>), see <a href="#vorlagen">Ready-made templates</a>. The difference: the system prompt applies always and unasked, a template is chosen deliberately. The wording of the rules lives in <code>SERVER_INSTRUCTIONS</code> (<code>src/afa_mcp/server.py</code>), the templates in <code>src/afa_mcp/prompts.py</code>, the reasoning in <a href="https://github.com/Archiv-fur-Agrargeschichte/AfA-MCP-Server/tree/main/docs/prompts">docs/prompts</a>.</p>',
    nav_intro: 'Introduction',
    ...
    reliability: '<p>AI assistants are a research tool, not a source. As a historian, keep two properties in mind:</p><p><strong>Reproducibility:</strong> ask the same question twice and the assistant may formulate different queries, select different hits and set different emphases in its summary. What you find today, a fellow student may not find tomorrow with the same question. Therefore record what matters: the document IDs and the links to the source entries. These are stable and citable, the chat transcript is not.</p><p><strong>What helps.</strong> The more precise the input, the more repeatable the result. A question in plain language leaves many decisions to the assistant (tool, search terms, filters). An explicitly noted query such as <code>search_entities({"entity_type": "person", "query": "Ineichen"})</code> pins these decisions down and returns the same hits as long as the holdings are unchanged. Recommendation: ask freely while browsing around, but for key findings copy the actually executed query from the conversation and document it.</p><p>If you need full reproducibility, call the interface directly via script (see the technical reference).</p><p>In short: the assistant replaces the search form, not source criticism.</p>',
    ...
    integration_p: 'Click the respective client below for the detailed instructions.',
    ...
    tool_fetch: 'Metadata of a single document via its ID.',
