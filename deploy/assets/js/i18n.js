...
const I18N = {
  de: {
    ...
    call_intro: 'Für diese Anfrage ruft der KI-Client dieses Werkzeug auf:',
    more_tools_h: 'Weitere Werkzeuge',
    tools_intro: '<p>Ohne spezifische Anweisung berechnet der KI-Client die Antwort unter Zuhilfnahme aller Werkzeuge, die vom MCP-Server zur Verfügung gestellt werden.</p>',
    ...
    ex1_tool: 'Mehr zum Werkzeug search_entities mit entity_type person',
    ...
    ex2_tool: 'Mehr zum Werkzeug search_entities mit entity_type institution',
    ...
    ex3_tool: 'Mehr zum Werkzeug search_audiovisual',
    ...
    ex4_tool: 'Mehr zum Werkzeug search_edition_gillabert_randin bzw. search_edition_hofstetter bzw. search_edition_bobbett',
    ex5_h: 'Volltext von Einträgen konsultieren',
    ex5_q: '«Volltext des Eintrags zu Ineichen, Franz (1887-1953)--DB1734 wiedergeben und die dort angegebenen Quellenangaben nennen.»',
    ex5_tool: 'Mehr zum Werkzeug fetch_document',
    nav_clients: 'Clients',
    nav_examples: 'Werkzeuge',
    nav_tips: 'Nutzung',
    nav_reliability: 'Reproduzierbarkeit',
    ...
    intro: 'Die AGHIST-Suche erfasst sämtliche Online-Ressourcen, die vom Archiv für Agrargeschichte öffentlich und unentgeltlich zur Verfügung gestellt werden. Es handelt sich um mehr als 15.000 Einträge zu Personen, mehr als 1.200 Einträge zu Institutionen, Einträge zu landwirtschaftlichen Betrieben, Einträge zu audio-visuellen Quellen und zu Verzeichnungsdaten von Archivbeständen sowie digitale Editionen und Publikationen aus dem AfA und über das AfA. Das AGHIST <a href="https://www.recherche2.histoirerurale.ch">Recherche-Portal</a> des <a href="https://histoirerurale.ch">Archivs für Agrargeschichte</a> ermöglicht einerseits eine syntaktische (textuelle) Suche. Über den MCP-Server (Model Context Protocol, Streamable HTTP) sind diese Online-Ressourcen auch aus Claude, ChatGPT, Cursor, Perplexity und anderen MCP-fähigen Clients ansprechbar.',
    guide_h: "So funktioniert die Recherche",
    guide_p: '<p>Ist der Konnektor zum MCP-Server eingerichtet und Verbindung hergestellt und für die Suche aktiviert, stellen Sie Recherchefragen in natürlicher Sprache: Der KI-Client durchsucht die Online-Ressourcen, kombiniert Suchanfragen, folgt Querverweisen und fasst die Ergebnisse zusammen. Zum Beispiel:</p>'
    ...
    holdings: '<table><thead><tr><th>Bestand</th><th>Inhalt</th></tr></thead><tbody>'
        ...
        + '<tr><td><strong>Digitale Editionen</strong></td><td>Drei digitale Editionen mit Texten von und zu Mina Hofstetter, Augusta Gillabert-Randin und Elizabeth Bobbett</td></tr>'
    ...
    setup_steps: '<ol><li><strong>KI-Client auswählen.</strong> Sie benötigen einen Zugang bspw. zu Claude (claude.ai oder Desktop-App), ChatGPT (Plus/Team/Enterprise), Perplexity Pro oder einer Entwicklungsumgebung wie Cursor oder VS Code.</li><li><strong>Server verbinden (Konnektor hinzufügen, App hinzufügen).</strong> Tragen Sie die Adresse des Endpoints <code>https://mcp.histoirerurale.ch/mcp</code> als MCP-Server ein. Die genauen Schritte unterscheiden sich je nach KI-Client, siehe die Anleitungen im nächsten Abschnitt.</li><li><strong>Start.</strong> Stellen Sie eine Rechercheanfrage in natürlicher Sprache. Eine Suchsyntax kann zusätzlich verwendet werden.</li></ol>',
    examples_h: 'Werkzeuge des MCP-Servers',
    tips_h: 'Nutzung des MCP-Servers',
    tips: '<ul><li><strong>Iterativ arbeiten.</strong> Beginnen Sie mit einer breit gefassten Anfrage und verengen Sie diese erst in einem zweiten Schritt: erst «Landfrauenverbände», dann «nur Kanton Bern», dann «Volltext von Eintrag X».</li><li><strong>Nach Querverweisen fragen.</strong> Insbesondere im Portal «Personen und Institutionen» sind die Einträge untereinander vernetzt (Verwandtschaft, Mitgliedschaften, Vorgänger und Nachfolger von Funktionärinnen und Funktionären, etc.). Fragen Sie nach diesen Verbindungen: «Welche weiteren Personen sind mit dem Eintrag Schnyder, Bertha (1887-1968)--DB3219 verknüpft?»</li><li><strong>Mehrsprachig suchen.</strong> Die Online-Ressourcen sind mehrsprachig, Prompts können wie folgt ergänzt werden: «Suche auch auf Englisch und Französisch».</li><li><strong>Quellen verifizieren.</strong> Für Zitate immer den Quell-Eintrag über die mitgelieferte Live-URL bzw. den dort angebebenen Zitiervorschlag angeben.</li><li><strong>Scope berücksichtigen.</strong> Der KI-Client findet nur Texte, die im AGHIST-Portal hinterlegt sind.</li><li><strong>Suchwege des KI-Clients nachvollziehen.</strong> Viele KI-Clients zeigen im Chatverlauf an, welche Suchanfragen sie tatsächlich ausgeführt haben (meist als aufklappbare Blöcke wie «Used afa-recherche search»). Konsultieren Sie diese Suchprotokolle, um die Abfrage etwas besser nachvollziehen zu können. So sehen Sie, mit welchen Begriffen und Filtern gesucht wurde, und Sie können u.U. erkennen, ob eine Recherche zu eng oder zu breit angelegt war. Die Zusammenfassung des Clients ist nicht dasselbe wie das Suchergebnis.</li></ul>',
    reliability_h: "Reproduzierbarkeit der Abfrageergebnisse",
    reliability: '<p>KI-Clients sind ein (mehr oder weniger zuverlässiges) Recherchewerkzeug, keine Quelle. Stellen Sie dieselbe Frage zweimal (das zweite Mal u.U. im Inkognito-Modus), kann der KI-Client unterschiedliche Suchanfragen an den MCP-Server formulieren, andere Treffer auswählen und zu anderen Schwerpunkten in der Zusammenfassung kommen. Was Sie heute finden, findet eine andere Person morgen mit derselben Frage vielleicht nicht. Die Dokument-IDs und die Links zu den Quell-Einträgen sind stabil und zitierfähig, der Chatverlauf ist es nicht.</p><p>Je enger geführt die Eingabe ist, desto eher kann das Ergebnis reproduziert werden. Eine Frage in natürlicher Sprache lässt dem KI-Client viel Spielraum bei der Auswahl der Werkzeuge, der Suchbegriffe und der Filter. Ein explizit notierter Suchaufruf wie <code>search_entities({"entity_type": "person", "query": "Ineichen"})</code> legt diese Entscheidungen fest und liefert bei unverändertem Datenbestand auf dem MCP-Server dieselben Treffer. Empfehlung: Für die Nachvollziehbarkeit den tatsächlich ausgeführten Suchaufruf aus dem Chatverlauf kopieren und dokumentieren.</p><p>Wer Reproduzierbarkeit benötigt, ruft die Schnittstelle direkt per Skript auf (siehe technische Dokumentation).</p><p>Kurz: Der KI-Client ergänzt die syntaktische Suche des AGHIST-Suchportals, die Prüfung und Würdigung der Quellen ist eine separate Aufgabe.</p>',
    techref_h: "Technische Dokumentation",
    techref_p: 'Dieser Abschnitt richtet sich an Entwicklerinnen und Entwickler sowie an alle, die erfahren möchten, wie ein KI-Client auf dem MCP-Server sucht.',
    ...
    direct_intro: '<p>Für technisch interessierte Nutzerinnen und Nutzer ist der MCP-Server auch ohne KI-Client nutzbar, ähnlich wie eine herkömmliche Schnittstelle (API). MCP basiert auf JSON-RPC über HTTP: Jedes Werkzeug lässt sich direkt aufrufen, etwa per <code>curl</code> oder aus einem Skript (siehe «eigene MCP-Clients»). Das ist der Weg zu voller Reproduzierbarkeit: gleicher Aufruf, gleiche Treffer, kein zwischengeschaltetes Sprachmodell.</p>',
    ...
    integration_p: 'Klicken Sie unten auf den jeweiligen KI-Client für die detaillierte Anleitung.',
    ...
    tool_server_info_params: '<p>Keine Parameter.</p>'
        ...
        + '</tbody></table>',
    tools_note: 'Volltext-Suche und Hierarchie-Filter lassen sich beliebig kombinieren, z.B. <code>search(query="Alp", hierarchy=["AfA_FotoFilm"])</code> für Alp-Bilder. Mehrere Hierarchie-IDs werden mit OR verknüpft, mit der Volltext-Suche per AND.',
    ...
    poweredby: 'powered by <a href="https://histoirerurale.ch">AfA</a> und <a href="https://www.pansoft.de/">Pansoft GmbH</a>'
  },
  fr: {
