const I18N = {
  ...
  it: {
    ...
    sysprompt: '<p>Un server MCP non fornisce al client soltanto strumenti, ma anche un testo di istruzioni (campo <code>instructions</code>) consegnato a ogni connessione. Il client lo anteponde al modello linguistico e influisce quindi sulla cura con cui un assistente lavora, senza che l\'utente debba configurare nulla.</p><p>Il server AfA vi descrive i fondi e gli strumenti e trasmette dieci regole di lavoro volte a ridurre la dispersione delle risposte: documentare ogni affermazione con identificatore e link, non rispondere partendo dall\'estratto abbreviato ma consultare la voce, ordinare per identificatore invece che per pertinenza per risultati ripetibili, scorrere l\'elenco fino in fondo prima di parlare di «tutti i risultati», elencare tutte le cariche riportate nella voce e non le principali, e indicare le contraddizioni fra voci con entrambi gli identificatori invece di risolverle. Anche così selezione e formulazione dell\'assistente restano variabili: le regole riducono omissioni e casualità, non le eliminano.</p><p>A complemento, e utilizzabili ricerca per ricerca, ci sono i modelli di prompt citati nella sezione sull\'affidabilità: formato di ricetta con somma di controllo, modelli di lavoro e verifica esterna. Il testo delle regole si trova in <code>SERVER_INSTRUCTIONS</code> (<code>src/afa_mcp/server.py</code>), la motivazione e i modelli in <a href="https://github.com/Archiv-fur-Agrargeschichte/AfA-MCP-Server/tree/main/docs/prompts">docs/prompts</a>.</p>',
