...
def test_describe_tools_call_search():
    payload = {
        ...
        "params": {
            "name": "search",
            "arguments": {"query": "Baeuerin Emmental", "size": 3},
        },
    }
    [r] = _describe_rpc(payload)
    assert r["method"] == "tools/call"
    assert r["tool"] == "search"
    assert r["args"] == {"query": "Baeuerin Emmental", "size": 3}
...
def test_describe_batch():
    ...
    assert rs[1]["id"] is None  # Notification, keine ID
...
def test_summarize_args_truncation(monkeypatch):
    monkeypatch.setenv("AFA_ACCESS_LOG_ARGS_MAX", "20")
...
# ---------------------------------------------------------------------------
# Middleware-Integration: prueft, dass der Body unveraendert ankommt und
# die Logzeile geschrieben wird.
# ---------------------------------------------------------------------------


async def _drive_middleware(payload: dict[str, Any] | list[dict[str, Any]]):
    body = json.dumps(payload).encode("utf-8")
    received_body = bytearray()

    async def inner_app(scope, receive, send):
        # Body komplett einlesen wie ein echter Handler.
        while True:
            msg = await receive()
    ...
    # Body in zwei Chunks aufteilen, damit der more_body-Pfad getestet wird.
    half = len(body) // 2
    requests = [
...
def test_middleware_replays_body_intact(caplog):
    ...
    payload = {
        ...
        "id": 7,
        "method": "tools/call",
        "params": {"name": "search_edition_bobbett", "arguments": {"query": "Wicklow"}},
    }
    received, sent = asyncio.run(_drive_middleware(payload))
    assert json.loads(received) == payload, "Body kommt am Handler unveraendert an"
    assert sent[0]["status"] == 200
    # Logzeile vorhanden + relevante Felder?
    line = next(r.getMessage() for r in caplog.records if r.name == "afa_mcp.access")
    assert "method=tools/call" in line
    assert "tool=search_edition_bobbett" in line
    assert "id=7" in line
...
def test_middleware_logs_batch_separate_lines(caplog):
    ...
    asyncio.run(_drive_middleware(payload))
    lines = [r.getMessage() for r in caplog.records if r.name == "afa_mcp.access"]
...
def test_middleware_skips_non_mcp_path(caplog):
    ...
    assert sent[0]["status"] == 200
    assert not [r for r in caplog.records if r.name == "afa_mcp.access"]
