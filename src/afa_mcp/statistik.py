"""Statistik-Endpoint + JSON-Tagescache fuer den AfA-MCP-Server.

Verarbeitet das Access-Log (``~/afa-mcp/afa-mcp.log``) und liefert eine
selbst-enthaltene HTML-Seite mit Tageszahlen, KI-Client-Klassifizierung,
Top-Tools, Methoden-Verteilung und Stunden-Sparklines. Die Seite wird
**bei jedem Aufruf live** generiert; Vortage werden in einer JSON-Cache-
Datei festgehalten (``~/afa-mcp/stats-cache.json``), so dass beim Aufruf
nur der laufende Tag aus dem Log neu aggregiert werden muss.

Konfiguration via Env-Variablen:

* ``AFA_ACCESS_LOG_FILE`` — Pfad zum Access-Log (Default ``~/afa-mcp/afa-mcp.log``)
* ``AFA_STATS_CACHE``     — Pfad zum JSON-Cache (Default ``~/afa-mcp/stats-cache.json``)
* ``AFA_STATS_USER``      — Basic-Auth-Benutzer (leer = Auth aus)
* ``AFA_STATS_PASS``      — Basic-Auth-Passwort
"""
...
log = logging.getLogger("afa_mcp.statistik")
...
def iter_log_lines(path: Path) -> Iterable[str]:
    """Hauptlog plus rotierte Geschwister (``*.1``, ``*.2.gz``) chronologisch."""
    parent = path.parent
    base = path.name
...
# ---------------------------------------------------------------------------
# Cache I/O — Counter <-> dict, atomares Schreiben
# ---------------------------------------------------------------------------
...
def refresh_cache(cache_path: Path, log_path: Path,
    ...
    """Kombiniert Cache mit Live-Log-Aggregaten.

    Vortage werden im Cache fixiert (idempotent). Der heutige Tag wird stets
    live aus dem Log berechnet — Cache fuer ihn wird nicht persistiert.
    """
...
# ---------------------------------------------------------------------------
# Aggregator-View ueber alle Tage (fuer Top-Tools, KI-Clients, Methoden)
# ---------------------------------------------------------------------------
...
def overall_apps(days: dict[str, dict]) -> dict[str, dict]:
    """Pro KI-Client: Sessions (initialize-Counts) + letzte Sitzung (Max)."""
...
# ---------------------------------------------------------------------------
# HTML-Rendering (unveraendert in Optik zur statischen Variante)
# ---------------------------------------------------------------------------
...
body { margin: 0; font-family: Georgia, "Times New Roman", serif;
  background: radial-gradient(circle at top left, rgba(138,154,91,.16), transparent 28rem),
...
<style>{css}</style></head><body><main>
  ...
  <div class="panel"><h2>Top-Tools</h2>
    <p class="lead">Welche AfA-Werkzeuge wurden tatsächlich aufgerufen?</p>{tool_table}</div>
  ...
  <div class="panel"><h2>Tagesübergreifende Aktivität nach Stunde</h2>
    <p class="lead">Tool-Aufrufe (dunkelgrün) und Setup (heller) über alle Tage, Server-Zeit.</p>{hours_chart}</div>
  ...
  <footer>Live-Generierung bei jedem Aufruf. Vortage in JSON-Cache fixiert.
    ...
    (<code>AFA_ACCESS_LOG_ARGS_MAX=0</code>).</footer>
</main></body></html>
"""
...
def _unauthorized() -> Response:
    return Response(
        ...
        headers={"WWW-Authenticate": 'Basic realm="AfA-MCP Statistik"'},
    )


def check_basic_auth(request: Request) -> Optional[Response]:
    user = os.environ.get("AFA_STATS_USER", "").strip()
    password = os.environ.get("AFA_STATS_PASS", "")
    if not user or not password:
        return None  # Auth aus (Dev-Modus)
...
async def statistik_endpoint(request: Request) -> Response:
    ...
    if auth is not None:
        return auth
    log_path = _env_path("AFA_ACCESS_LOG_FILE", DEFAULT_LOG)
    cache_path = _env_path("AFA_STATS_CACHE", DEFAULT_CACHE)
